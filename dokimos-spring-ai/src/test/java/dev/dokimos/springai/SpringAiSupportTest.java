...
class SpringAiSupportTest {
    ...
    void asJudge_withChatClientBuilder_shouldDelegateToChatClient() {
        ChatModel mockModel =
                prompt -> new ChatResponse(List.of(new Generation(new AssistantMessage("Judge response"))));
    ...
    void asJudge_withChatModel_shouldDelegateToChatModel() {
        ChatModel mockModel =
                prompt -> new ChatResponse(List.of(new Generation(new AssistantMessage("Model response"))));
    ...
    void toTestCase_shouldMapUserTextToInput() {
        EvaluationRequest request =
                new EvaluationRequest("What is the capital of France?", List.of(), "Paris is the capital of France.");
    ...
    void toTestCase_shouldMapResponseContentToActualOutput() {
        EvaluationRequest request = new EvaluationRequest("Question", List.of(), "The answer is 42.");
    ...
    void toTestCase_shouldMapDocumentsToContext() {
        ...
        EvaluationRequest request = new EvaluationRequest("Question", documents, "Answer based on documents");
        ...
        assertThat(context).containsExactly("Document 1 content", "Document 2 content", "Document 3 content");
    }

    @Test
    void toTestCase_shouldHandleEmptyDocumentList() {
        EvaluationRequest request = new EvaluationRequest("Question", List.of(), "Answer");
    ...
    void toTestCase_shouldHandleNullDocumentList() {
        EvaluationRequest request = new EvaluationRequest("Question", null, "Answer");
    ...
    void toEvaluationResponse_shouldMapSuccessToPass() {
        EvalResult successResult = EvalResult.success("faithfulness", 0.95, "Response is faithful to context");
    ...
    void toEvaluationResponse_shouldMapFailureToNotPass() {
        EvalResult failureResult = EvalResult.failure("faithfulness", 0.3, "Response contains hallucinations");
    ...
    void toEvaluationResponse_shouldMapMetadata() {
        EvalResult result = new EvalResult(
                ...
                true,
                "Evaluation passed",
                Map.of("model", "gpt-5", "temperature", 0.7, "tokens", 150));
    ...
    void toEvaluationResponse_shouldHandleEmptyMetadata() {
        EvalResult result = EvalResult.success("evaluator", 0.8, "Good");
    ...
    void roundTripConversion_shouldPreserveData() {
        List<Document> documents = List.of(new Document("Context document 1"), new Document("Context document 2"));

        EvaluationRequest request =
                new EvaluationRequest("What is RAG?", documents, "RAG stands for Retrieval-Augmented Generation");

        EvalResult result =
                new EvalResult("faithfulness", 0.9, true, "Response is faithful", Map.of("confidence", 0.95));
